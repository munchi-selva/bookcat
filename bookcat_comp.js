...
function getDateComponent()
{
    ...
    DateComp.prototype.init = function(params)
    {
        ...
        this.dateControl.setDate(this.params.formatValue(params.value));
    };
    ...
    // Focus on the date input so that editing can be done
    DateComp.prototype.afterGuiAttached = function()
    {
        this.gui.querySelector(".date-input").focus();
    };
...
function getDateControl()
{
    ...
    DateControl.prototype.init = function()
    {
        ...
        this.root.classList.add("date-input-wrapper");
        this.root.innerHTML = "<div class='grid-container' style='grid-template-columns: auto 20%'>" +
    ...
    DateControl.prototype.setupGuiHooks = function()
    {
        ...
        let dateInput       = this.root.querySelector(".date-input");
        let hiddenDateInput = this.root.querySelector(".hidden-date-input");
        let launcherBtn     = this.root.querySelector(".btn-date-launcher");

        // Set up a change event handler on the visible date input
        dateInput.addEventListener(DATE_CHANGE_EVENT_TYPE, dateInputChanged);
        ...
        let datePicker = flatpickr(hiddenDateInput,
                                   {
                                        ...
                                        {
                                            dateControl.raiseDateInputChange(dateInput, dateStr, false);
                                        }
                                    });
        ...
        // 1. [Optional] Synchs the manual date input and date picker, if required
        // 2. Propagates (bubbles up) a change event to the parent
        function dateInputChanged(event)
        {
            ...
            let propagatedChangeEvent = new Event(DATE_CHANGE_EVENT_TYPE, {bubbles: true});
            target.parentElement.dispatchEvent(propagatedChangeEvent);
        }
    }
    DateControl.prototype.setDate = function(dateStr)
    {
        let dateInput = this.root.querySelector(".date-input");
        this.raiseDateInputChange(dateInput, dateStr, true);
    }

    DateControl.prototype.getDate = function()
    {
        return this.root.querySelector(".date-input").value;
    }

    // 1. [Optional] Sets the value of the manual date input
    // 2. Raises a change event on the manual date input
    DateControl.prototype.raiseDateInputChange = function(dateInput, dateStr, updateDatePicker)
    {
...
function getDateFilterComponent()
{
    ...
    DateFilterComponent.prototype.init = function(params)
    {
        ...
        this.startDateControl.root.classList.add("start-date");

        this.endDateControl = getDateControl();
        this.endDateControl.root.classList.add("end-date");

        let selectElem = document.createElement("select");
        let selectInner = "";
        for (let idx = 0; idx < filterTypes.length; idx++)
        {
            selectInner += "<option value='" + filterTypes[idx].ftVal + "'>" +
                             filterTypes[idx].ftLabel +
                          "</option>";
        }
        selectElem.innerHTML = selectInner;

        let buttonDiv = document.createElement("div");
        buttonDiv.style.setProperty("text-align", "right");
        buttonDiv.innerHTML = "<button class='btn-filter-clear'>Clear</button>" +
                              "<button class='btn-filter-reset'>Reset</button>";

        this.gui.appendChild(selectElem);
        this.gui.appendChild(this.startDateControl.root);
        this.gui.appendChild(this.endDateControl.root);
        this.gui.appendChild(buttonDiv);
    ...
    DateFilterComponent.prototype.setupGuiHooks = function(params)
    {
        ...
        this.startDateControl.root.addEventListener(DATE_CHANGE_EVENT_TYPE, function(event) { params.filterChangedCallback(); });
        this.endDateControl.root.addEventListener(DATE_CHANGE_EVENT_TYPE, function(event) { params.filterChangedCallback(); });
        ...
        {
            filterComp.startDateControl.setDate(DEFAULT_DATE_STR);
            filterComp.endDateControl.setDate(DEFAULT_DATE_STR);
        }
    }
